...
namespace irimhe.Models
{
    public class CatalTModel
    {
        ...
        public string only_temp(string station)
        {
            ...
            {
                sql = "select * from agro_catal_t";
            }
            else
            {
                int sta = Int32.Parse(station);
                sql = "select * from agro_catal_t index ="+sta+"";
            }
        ...
        public List<catalclass> catalt(DataTable data)
        {
            catalclass temp = new catalclass();

            var convertedList = (from rw in data.AsEnumerable()
                                 select new catalclass()
                                 {
        ...
        public string GetCatalP_station(int station)
        {
            ConnDB conn = new ConnDB();
